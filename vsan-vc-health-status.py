...
def ConnectToHealthService(stub):
   vhStub = CreateVsanStub(stub)
   return vim.VsanVcClusterHealthSystem('vsan-cluster-health-system', vhStub)

def ConnectToObjectService(stub):
   vhStub = CreateVsanStub(stub)
   return vim.cluster.VsanObjectSystem('vsan-cluster-object-system', vhStub)

def CreateVsanStub(stub):
...
if selectedMode == 'all' or selectedMode == 'cluster-health':
   for cluster in clusters:
      ...
      if not vsanEnabled:
         ...
         try:
            res = ReadVsanHwToVcgInfoMappings(cluster)
            print("vSAN hardware VCG mappings in VCDB: %s" % str(res))
         except Exception as ex:
            print("Failed to dump VCG mappings: %s" % str(ex))
      print("")
      print("")

if selectedMode == 'other-type-objects':
   vcos = ConnectToObjectService(conn)
   for cluster in clusters:
      vsanConfig = clustersProps[cluster]['configurationEx'].vsanConfigInfo
      vsanEnabled = vsanConfig is not None and vsanConfig.enabled
      if not vsanEnabled:
         ...
         try:
            otherObjects = vcos.VsanQueryObjectIdentities(cluster,
                              objTypes = ['other'],
                              includeHealth = False)
            print("Other Type objects: \n%s" % (str(otherObjects)))
         except Exception as ex:
            print("Failed to gather object Identities: %s" % str(ex))
      print("")
      print("")
