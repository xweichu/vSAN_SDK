...
function ErrorMessage(props) {
    props = props || {};
    return (targetClass, propertyKey) => {
        const loaderName = props.name;
        setErrorMessageGetter(loaderName, targetClass, propertyKey);
        Object.defineProperty(targetClass, propertyKey, {
            get: function () {
                const exception = getException.call(this, loaderName);
...
function getException(loaderName) {
    const singleLoaderName = Array.isArray(loaderName)
...
function setException(loaderName, val) {
    if (Array.isArray(loaderName)) {
        ...
        loaderName.forEach(name => (new _service_flow_loading_flow_wrapper_data__WEBPACK_IMPORTED_MODULE_0__["LoadingFlowWrapper"](this, name)).error = val);
    }
    else {
