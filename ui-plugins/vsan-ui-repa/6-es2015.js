...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "PerfChartUtil", function() { return PerfChartUtil; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "PerfChartDataConstants", function() { return PerfChartDataConstants; });
/* harmony import */ var _angular_common__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @angular/common */ "SVse");
/* harmony import */ var _component_chart_model_metric_comparisson_type_data__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @component/chart/model/metric-comparisson-type.data */ "w4Wc");
/* harmony import */ var _generated_perf_graph_threshold__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @generated/perf-graph-threshold */ "/EAF");
/* harmony import */ var _generated_perf_graph_threshold_direction__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! @generated/perf-graph-threshold-direction */ "fNdd");
...
/* harmony import */ var _util_vsan_dateteime_util__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! @util/vsan-dateteime-util */ "MAgC");
...
/* harmony import */ var _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ../../../../performance/model/perf-monitor-view-type.model */ "NvCx");
/* harmony import */ var _performance_view_perf_vm_mode_selector_perf_vm_mode__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! ../../../../performance/view/perf-vm-mode-selector/perf-vm-mode */ "FAE6");
/* harmony import */ var _perf_graph__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! ./perf-graph */ "lXqx");
...
let PerfChartUtil = /*@__PURE__*/ (() => {
    class PerfChartUtil {
        static getConsolidatedChartOptions(config, afterSetExtremesXAxisCallback, customizableThreshold, onMouseOver, onMouseOut) {
            const series = [];
            config.consolidatedMetrics
                .forEach((entry) => {
                if (entry.isLegend) {
                    ...
                    entry[PerfChartUtil.LEGEND_COLOR] = config.entityIdentifierToColor[entry.entityIdentifier.id];
                    // Map to the legend item so that clicking on the legend entry toggles the chart lines
            ...
            const consolidatedChartOptions = PerfChartUtil.getBasicChartOptions(config.graph, this.getShortestSeriesLength(series), config.legendPosition);
            ...
            consolidatedChartOptions.xAxis = PerfChartUtil.getXAxisOptions(config.graph, config.firstEntityPerfData &&
                config.firstEntityPerfData.timeStamps, config.looseXAxisLabel, afterSetExtremesXAxisCallback);
            consolidatedChartOptions.yAxis =
                PerfChartUtil.getYAxisOptions(config.graph, config.firstEntityPerfData, series, customizableThreshold);
         ...
         */
        static getChartOptions(entityRefId, graphConfig, metrics, index = 0, looseXAxisLabel, legendPostion, afterSetExtremesXAxisCallback, customizableThreshold, keepPlotLineOnClick, onPlotLineClicked, actionLink, seriesMouseOverCallback, seriesMouseOutCallback) {
            const series = PerfChartUtil.getChartSeries(graphConfig, metrics);
            let basicOptions = PerfChartUtil.getBasicChartOptions(graphConfig, this.getShortestSeriesLength(series), legendPostion, keepPlotLineOnClick, onPlotLineClicked);
            ...
            const additionalGraph = graphConfig.secondGraph;
            /*
            ...
            */
            ...
            basicOptions.tooltip = {
                ...
                // html formatters
                headerFormat: graphConfig.isHistogram
                    ? PerfChartUtil.getHistogramHeaderFormat(graphConfig)
                    : null,
                pointFormatter: graphConfig.isHistogram
                    ? PerfChartUtil.getHistogramPointFormatter(graphConfig)
                    : null,
                footerFormat: graphConfig.isHistogram
                    ? PerfChartUtil.getHistogramFooterFormat(graphConfig)
                    : null,
                // non html formatter
                formatter: graphConfig.isHistogram
                    ? null
                    : PerfChartUtil.getSeparatedTooltipFormatter(graphConfig, metrics, actionLink),
                shared: true,
                useHTML: true,
        ...
        static getShortestSeriesLength(series) {
            let shortestSeriesLength = Number.MAX_VALUE;
            // compare against the length of all not nullable values in the series
            series === null || series === void 0 ? void 0 : series.forEach(s => {
                var _a;
                const seriesLength = (_a = s === null || s === void 0 ? void 0 : s.data) === null || _a === void 0 ? void 0 : _a.filter(data => !_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].isUnset(data)).length;
                if (seriesLength) {
        ...
        static getHistogramHeaderFormat(graphConfig) {
            const hasCategories = PerfChartUtil.getHistogramChartCategories(graphConfig) !== null;
            if (hasCategories) {
                return "<table>";
            }
            else {
        ...
        static getConsolidatedHeaderFormat(graphConfig) {
            return "<div>{point.key}</div>";
        }
        static getHistogramPointFormatter(graphConfig) {
            const hasCategories = PerfChartUtil.getHistogramChartCategories(graphConfig) !== null;
            return function (tooltip) {
                const value = _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.perf.chart.label.unit.percentage", _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].localizeNumber(PerfChartUtil.getAsPercentages(this.y, graphConfig.unit)));
                if (hasCategories) {
                    return `<tr>
                     <td>
        ...
        static getConsolidatedPointFormatter(graph) {
            return function (tooltip) {
                if (this.isNull) {
                    return _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.na.label");
                }
                let selectedSeriesTooltipContent = "<table>";
                const selectedSeries = this.series;
                /* Set tooltip content for hovered metric */
                const entityName = PerfChartUtil.escapeAngledBrackets(selectedSeries.userOptions.entityIdentifier.name);
                const metricName = PerfChartUtil.escapeAngledBrackets(selectedSeries.userOptions.metricName);
                const value = PerfChartUtil.formatYAxisLabel(selectedSeries.processedYData[this.index], graph);
                let relatedPointsTooltipContent = "";
                const relatedSeries = [];
                /*
                ...
                */
                this.series.chart.series.forEach((seriesEntry) => {
                    if (selectedSeries === seriesEntry || selectedSeries.name === seriesEntry.name ||
                        seriesEntry.userOptions.isLegend || !seriesEntry.visible ||
                        relatedSeries.includes(seriesEntry.name)) {
                        return;
                    }
                    const pointY = seriesEntry.processedYData[this.index];
                    const diffY = Math.abs(seriesEntry.processedYData[this.index] - this.y);
                    /*
                     Check if current series is 0.1 max away from hovered point
                     */
                    if (diffY <= 0.1) {
                        const valueY = pointY != null
                            ? PerfChartUtil.formatYAxisLabel(pointY, graph)
                            : _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.na.label");
                        const entityNameSeriesEntry = PerfChartUtil.escapeAngledBrackets(seriesEntry.userOptions.entityIdentifier.name);
                        const metricNameSeriesEntry = PerfChartUtil.escapeAngledBrackets(seriesEntry.userOptions.metricName);
        ...
        static getSeriesTooltipContent(relatedSeries, entityName, metricName, value, color) {
            return `<tr>
                  <td ${relatedSeries.length > 0
                ? "class=\"tooltip-padding-bottom\""
                : ""}>
                     <svg class="tooltip-color-flag" style="fill:${color}">
                        <rect x="0" y="4" width="25" height="6"/>
                     </svg>
                     <b>${entityName}</b> ${metricName}:</td>
                  <td ${relatedSeries.length > 0
                ? "class=\"tooltip-padding-bottom tooltip-value\""
                : "class=\"tooltip-value\""}>${value}</td>
               </tr>`;
        }
        static getHistogramFooterFormat(graphConfig) {
            const hasCategories = PerfChartUtil.getHistogramChartCategories(graphConfig) != null;
            if (hasCategories) {
                return "</table>";
            }
        ...
        static escapeAngledBrackets(text) {
            return String(text)
                .replace(/</g, "&lt;")
                .replace(/>/g, "&gt;");
        }
        static getSecondGraphOptions(options, primaryGraph, primaryGraphMetrics, primaryGraphSeries, secondaryGraph, looseXAxisLabel, afterSetExtremesXAxisCallback, customizableThreshold) {
            // Get additional graph series
            let secondYAxisSeries = PerfChartUtil.getChartSeries(secondaryGraph, primaryGraphMetrics);
            // Set yAxis index for secondary graph series
            secondYAxisSeries = secondYAxisSeries.map(seriesItem => {
                // Set yAxis index
                seriesItem.yAxis = 1;
                return seriesItem;
            });
            // Set yAxis index for primary graph series
            primaryGraphSeries = primaryGraphSeries.map(seriesItem => {
                // Set yAxis index
                seriesItem.yAxis = 0;
                return seriesItem;
            });
            // Build primary yAxis
            const primaryYAxis = PerfChartUtil.getYAxisOptions(primaryGraph, primaryGraphMetrics, primaryGraphSeries, customizableThreshold);
            primaryYAxis.title.text = primaryGraph.label;
            // Build secondary yAxis
            const secondaryYAxis = PerfChartUtil.getYAxisOptions(secondaryGraph, primaryGraphMetrics, secondYAxisSeries, customizableThreshold);
            // Set opposite side for second graph in order to display second graph scala on right
            secondaryYAxis.opposite = true;
            secondaryYAxis.gridLineWidth = 0;
            secondaryYAxis.title.text = secondaryGraph.label;
            // Set promary and secondary y axis to chart options
            options.yAxis = [primaryYAxis, secondaryYAxis];
            // Build warning areas based on graph type and series
            const warningAreaPolygons = PerfChartUtil.getWarningAreas(secondaryGraph, secondYAxisSeries);
            // Set warning areas series, primary graph series and secondary graph series
            if (warningAreaPolygons != null) {
                options.series = [...warningAreaPolygons.polygonSeries, ...primaryGraphSeries, ...secondYAxisSeries];
                // Add label for warning areas
                options.series.push(warningAreaPolygons.legendLabel);
            }
            else {
                options.series = [...primaryGraphSeries, ...secondYAxisSeries];
            }
            return options;
        }
        /*
        ...
        */
        static getWarningAreas(graph, series) {
            const chartMaximumValue = Number.MAX_SAFE_INTEGER;
            let spec;
            let legendLabel;
            /*
            ...
            */
            if (graph.id === "com.vmware.vsan.perf.graph.virtual-disk.normalized-iops") {
                const iopsLimitMetricName = graph.metrics.find(x => x.key === "iopsLimit").label;
                const normalizedIopsMetricName = graph.metrics.find(x => x.key === "NIOPS").label;
                const labelId = "iops-limit-latency";
                spec = {
                    labelId: labelId,
                    firstMetric: series.find(x => x.name === iopsLimitMetricName),
                    secondMetric: series.find(x => x.name === normalizedIopsMetricName),
            ...
            const polygonSeries = PerfChartUtil.buildAreaPolygons(spec);
        ...
        */
        static buildAreaPolygons(spec) {
            ...
            spec.secondMetric.data.push(undefined);
            // Build polygons with coordinates based on series values and operator
            spec.firstMetric.data.forEach((val, index) => {
                ...
                if (comparisonResult && index < spec.firstMetric.data.length - 1) {
                    ...
                    const endIndex = comparisonResult
                        ? index
                        : index - 1;
                    rightBottomPoint = {
                        x: endIndex,
        ...
        static getSeparatedTooltipFormatter(graphConfig, metrics, actionLink) {
            return function (tooltip) {
                const currentPage = tooltip.chart.legend.currentPage;
                const itemsPerPage = PerfChartUtil.legendItemsPerPage(tooltip);
                let s = `<div>${this.x}</div><table>`;
                /**
                ...
                this.points.forEach((point, index) => {
                    ...
                    let currentGraphConfig;
                    if (graphConfig.secondGraph != null &&
                        graphConfig.secondGraph.metrics.find(x => x.label === point.series.name)) {
                    ...
                    s += `<tr>
                     <td>
                        <svg class="tooltip-color-flag" style="fill: ${point.series.color}">
                           <rect x="0" y="4" width="25" height="6"/>
                        </svg>${PerfChartUtil.escapeAngledBrackets(point.series.name)}:
                     </td>
                     <td class="tooltip-value">
                        ${PerfChartUtil.formatYAxisLabel(point.y, currentGraphConfig, false, (_a = point.series.userOptions) === null || _a === void 0 ? void 0 : _a.metricKey)}
                     </td>
                  </tr>`;
                });
                if (actionLink) {
                    const timePoint = metrics.timeStamps[this.points[0].point.index];
                    s += `<td>
        ...
        static isInCurrentPage(index, currentPage, itemsPerPage) {
            if (itemsPerPage === -1) {
         ...
         */
        static legendItemsPerPage(tooltip) {
            const legend = tooltip.chart.legend;
            return !_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].isEmpty(legend.pages)
                ? Math.floor(legend.clipHeight / legend.itemHeight)
                : -1;
        }
        static getBasicChartOptions(graphConfig, shortestSeriesLength, legendPosition, keepPlotLineOnClick = false, onPlotLineClicked = null) {
            return {
                ...
                plotOptions: {
                    series: {
                        ...
                        point: {
                            events: {
                                click: function (e) {
                                    const _this = this; // "this" points to the hovered point object
                                    if (keepPlotLineOnClick) {
                                        highcharts__WEBPACK_IMPORTED_MODULE_9__["charts"].forEach(chart => {
        ...
        static addPlotLine(chart, valueIndex) {
            if (chart && chart.container && !_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].isEmpty(chart.series) && chart.series[0].xAxis) {
                const plotLineId = chart.container.id + "_plotline";
                const color = chart.series[0].xAxis.crosshair.color;
        ...
        static addSeriesEvent(plotOptionsSeries, mouseOverEvent, mouseOutEvent) {
        ...
        static getYAxisOptions(graphConfig, metrics, series, customizableThreshold) {
            const max = PerfChartUtil.getYAxisMaxVal(graphConfig, series, customizableThreshold);
            return {
                title: {
                    text: undefined,
                },
                ...
                plotBands: PerfChartUtil
                    .getThresholdBands(PerfChartUtil.getThreshold(graphConfig, metrics, customizableThreshold), max),
                max: max,
                tickInterval: max / 2,
            };
        }
        static getLegend(graph, legendPosition) {
            if (graph.isHistogram) {
                const hasCategories = PerfChartUtil.getHistogramChartCategories(graph) !== null;
                return hasCategories
                    ? PerfChartUtil.BOTTOM_LEGEND_OPTION
                    : PerfChartUtil.NO_LEGEND_OPTION;
            }
            return legendPosition === PerfChartUtil.LEGEND_RIGHT
         ...
         */
        static getThreshold(graph, metrics, customizableThreshold) {
        ...
        static getCustomizableThreshold(graph, customizableThresholds) {
            let threshold;
            if (!_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].isEmpty(customizableThresholds)) {
                ...
                for (const metric of graph.metrics) {
                    threshold = customizableThresholds.find(t => {
        ...
        static toPercentageThreshold(thresholdInPermille) {
            if (!thresholdInPermille) {
                return null;
            }
            const thresholdInPercentage = _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].deepClone(thresholdInPermille);
        ...
        static toPermilleThreshold(thresholdInPercentage) {
            if (!thresholdInPercentage) {
                return null;
            }
            const thresholdInPermille = _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].deepClone(thresholdInPercentage);
        ...
        static getAsPercentages(value, valueUnit) {
            return valueUnit === PerfChartUtil.PERMILLE_TYPE
                ? PerfChartUtil.toPercentages(value)
                : value;
        }
        static getThresholdFromMetrics(graph, metrics) {
            if (!metrics || _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].isEmpty(metrics.metricsSeries)) {
                return null;
            }
            const metricIds = new Set(graph.metrics.map(metric => metric.key));
            let threshold = null;
            for (const metric of metrics.metricsSeries) {
        ...
        static getThresholdBands(threshold, max) {
            const bands = [];
            if (!threshold) {
                return bands;
            }
         ...
         */
        static createBand(color, from, to) {
         ...
         */
        static formatYAxisLabel(data, graph, hasCustomizableThreshold = false, metricKey = null) {
            var _a;
            let unit = graph.unit;
            if (metricKey) {
                const metricUnit = (_a = graph.metrics.find(metric => metric.key === metricKey)) === null || _a === void 0 ? void 0 : _a.unit;
                if (metricUnit) {
                    unit = metricUnit;
                }
            }
         ...
         */
        static getYAxisMaxVal(graph, series, customizableThresholds) {
            ...
            if (series && series.length) {
                ...
                return max > 0
                    // Add 5% in addition to the max value, in order to allow hovering the max point.
                    ? max + max * .05
                    : PerfChartUtil.getNoValMax(graph);
            }
        }
        static isGraphInPermilles(graph, customizableThresholds) {
         ...
         */
        static getXAxisOptions(graphConfig, timeArray, looseXAxisLabel, afterSetExtremesXAxisCallback) {
            if (!timeArray || !timeArray.length) {
                return null;
            }
            const dateArr = timeArray.map(_util_vsan_dateteime_util__WEBPACK_IMPORTED_MODULE_7__["VsanDateTimeUtils"].convertDateToIso8601);
            const dateFormatter = new _angular_common__WEBPACK_IMPORTED_MODULE_0__["DatePipe"](_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getLocaleForDecimal());
            const formatPattern = this.getDateFormatPattern(dateArr, dateFormatter);
            /**
             ...
             */
            const tickInterval = looseXAxisLabel
                ? Math.floor((dateArr.length - 1) / 2)
                : Math.floor((dateArr.length - 1) / 4);
            return {
                categories: graphConfig.isHistogram
                    ? PerfChartUtil.getHistogramChartCategories(graphConfig)
                    : dateArr.map(date => dateFormatter.transform(date, formatPattern)),
                type: graphConfig.isHistogram
                    ? "category"
                    : null,
                labels: graphConfig.isHistogram
                    ...
                    : {},
                tickInterval: graphConfig.isHistogram
                    ? null
                    : tickInterval,
                crosshair: graphConfig.isHistogram
                    ? null
                    : {
                        width: 1,
                    },
         ...
         */
        static getDateFormatPattern(dates, dateFormatter) {
            const isSameDay = dateFormatter.transform(dates[0], PerfChartUtil.SHORT_DATE_PATTERN) ===
                dateFormatter.transform(dates[dates.length - 1], PerfChartUtil.SHORT_DATE_PATTERN);
            const startFromToday = dateFormatter.transform(dates[0], PerfChartUtil.SHORT_DATE_PATTERN) ===
                dateFormatter.transform(new Date(), PerfChartUtil.SHORT_DATE_PATTERN);
            const hasSeconds = dates === null || dates === void 0 ? void 0 : dates.some(date => date.getSeconds() > 0);
            let formatPattern = hasSeconds
                ? PerfChartUtil.MEDIUM_TIME_PATTERN
                : PerfChartUtil.SHORT_TIME_PATTERN;
            // When the start time and end time are not on the same day or when the time does not start from today,
            // the X-axis date is displayed.
            if (!isSameDay || !startFromToday) {
                formatPattern = hasSeconds
                    ? PerfChartUtil.SHORT_WITH_SECONDS_PATTERN
                    : PerfChartUtil.SHORT_PATTERN;
            }
            return formatPattern;
        }
        static getHistogramChartCategories(graphConfig) {
            const categories = new Set(graphConfig.metrics.map(metric => metric.group));
         ...
         *
         */
        static getChartSeries(graphConfig, perfEntityMetrics) {
            ...
            graphConfig.metrics
                // filter the unnecessary graphs (the graphs which don't have series data)
                .filter(gMetric => perfEntityMetrics.metricsSeries.some(metricsData => metricsData.key === gMetric.key))
                .forEach((gMetric) => {
                // consider it as a chart line definition to better understand the logic
                const lineSeries = perfEntityMetrics.metricsSeries.filter(metricsData => metricsData.key === gMetric.key);
                lineSeries.forEach(line => {
                    const matched = line && line.values.length
                        ? line.values
                        :
                            // return an array like [null, null, ..., null],
                            // the array must have same length with the timeStamps
                            // because highcharts throws exception if using empty array '[]'
                            perfEntityMetrics.timeStamps.map(() => null);
                    if (graphConfig.isHistogram) {
                        PerfChartUtil.createHistogramSeries(graphConfig, gMetric, matched[0], categories, chartSeries);
                    }
                    else {
                        const series = {
                            name: lineSeries.length === 1
                                ? gMetric.label
                                : PerfChartUtil.getSeriesLabel(gMetric.label, line.subKey),
                            data: matched,
        ...
        static createHistogramSeries(graphConfig, gMetric, metricValue, categories, chartSeries) {
            if (categories) {
                ...
                const group = graphConfig.metrics.find(metric => metric.key === gMetric.key).group;
                const groupIndex = categories.findIndex(category => category === group);
                ...
                const value = [gMetric.label, metricValue];
                if (chartSeries.length === 0) {
                    chartSeries.push({ data: [value] });
                }
                else {
        ...
        static createDefaultHistogramSeries(chartSeries) {
            if (chartSeries.length === 0) {
                chartSeries.push({
         ...
         */
        static getSeriesLabel(key, subKey) {
            return key + (subKey
                ? " " + subKey
                : "");
        }
        static isRealtimeSupportedView(selectedView) {
            return PerfChartUtil.HIGH_RESOLUTION_SUPPORTED_VIEWS.some(view => view === selectedView);
        }
        static getAggregatedVsanPerfVmMode(moRef) {
            return _service_managed_object__WEBPACK_IMPORTED_MODULE_5__["ManagedObjectUtils"].isHost(moRef)
                ? _performance_view_perf_vm_mode_selector_perf_vm_mode__WEBPACK_IMPORTED_MODULE_11__["VsanPerfVmMode"].HOST_AGGREGATION
    ...
    PerfChartUtil.CHART_HEIGHT = 200;
    // "short": "18/4/2018 12:05 PM"
    PerfChartUtil.SHORT_PATTERN = "short";
    // "show with seconds": "equivalent to 'M/d/yy, h:mm:ss a' 18/4/2018 12:05:33 PM.
    PerfChartUtil.SHORT_WITH_SECONDS_PATTERN = "M/d/yy, h:mm:ss a";
    // "shortTime": "12:05 PM"
    PerfChartUtil.SHORT_TIME_PATTERN = "shortTime";
    // "medium" : equivalent to 'h:mm:ss a' 9:03:01 AM.
    PerfChartUtil.MEDIUM_TIME_PATTERN = "mediumTime";
    // "shortDate": "18/4/2018"
    PerfChartUtil.SHORT_DATE_PATTERN = "shortDate";
    PerfChartUtil.WARNING_AREA_NAME = "Polygons";
    PerfChartUtil.HIGH_RESOLUTION_SUPPORTED_VIEWS = [
        _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].CLUSTER_VM_CONSUMPTION,
        _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].CLUSTER_BACKEND,
        _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].HOST_BACKEND,
        _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].HOST_VM_CONSUMPTION,
        _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].HOST_DISK,
        _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].HOST_PHYSICAL_ADAPTER,
        _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].HOST_VIRTUAL_ADAPTER,
    ];
    ...
    PerfChartUtil.toPercentages = (permilles) => permilles > 0
        ? permilles / 10
        : 0;
    PerfChartUtil.toPermilles = (percentages) => percentages * 10;
    PerfChartUtil.getGraphDefinition = (graph) => {
        const perfGraph = new _perf_graph__WEBPACK_IMPORTED_MODULE_12__["PerfGraph"]();
        ...
        perfGraph.metrics = graph.metrics.map(metrics => new _perf_graph__WEBPACK_IMPORTED_MODULE_12__["PerfGraphMetrics"](metrics.label, metrics.name, metrics.group, null, null, metrics.unit));
        return perfGraph;
    };
    return PerfChartUtil;
})();
let PerfChartDataConstants = /*@__PURE__*/ (() => {
    class PerfChartDataConstants {
    }
    ...
    PerfChartDataConstants.VSAN_MAX_DISK_LSOM = "com.vmware.vsan.perf.entity.vsan2-disk-lsom2";
    // historical space report view
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "VsanPerfVmMode", function() { return VsanPerfVmMode; });
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "BasePerfEntity", function() { return BasePerfEntity; });
/**
...
class BasePerfEntity {
    ...
    equals(that) {
        ...
        return this.entityId === that.entityId
            && this.entityUuid === that.entityUuid
            && this.configSpec === that.configSpec;
    }
}
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "PerfGraph", function() { return PerfGraph; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "PerfGraphMetrics", function() { return PerfGraphMetrics; });
/**
 * The definition of a perf chart
 */
let PerfGraph = /*@__PURE__*/ (() => {
    class PerfGraph {
        constructor() {
            /**
    ...
    PerfGraph.createPerfGraph = (id, label, unit, metrics, threshold, isPivotal = true) => {
        const graph = new PerfGraph();
        graph.id = id;
        graph.label = label;
...
class PerfGraphMetrics {
     ...
     */
    constructor(key, label, group, description, aggregatedBy, unit) {
        this.key = key;
        this.label = label;
        this.group = group;
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "MetricComparissonType", function() { return MetricComparissonType; });
