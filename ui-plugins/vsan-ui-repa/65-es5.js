(function () {
  ...
  (window["webpackJsonp"] = window["webpackJsonp"] || []).push([[65], {
    ...
    /***/
    function qp7v(module, exports, __webpack_require__) {
    ...
    /***/
    function u7SK(module, __webpack_exports__, __webpack_require__) {
      "use strict";
      ...
      __webpack_require__.d(__webpack_exports__, "IpParserService", function () {
        return IpParserService;
      });
      /* harmony import */


      var ipaddr_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(
      /*! ipaddr.js */
      "qp7v");
      /* harmony import */


      var ipaddr_js__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(ipaddr_js__WEBPACK_IMPORTED_MODULE_0__);
      /**
      ...
      var IpParserService = /*@__PURE__*/function () {
        var IpParserService = /*#__PURE__*/function () {
          function IpParserService() {
            var _this = this;

            _classCallCheck(this, IpParserService);

            this.isIpv4AddressValid = function (ipAddress) {
              var trimmedIp = ipAddress.trim();
              return ipaddr_js__WEBPACK_IMPORTED_MODULE_0__["IPv4"].isValid(trimmedIp) && _this.validIpV4Address(trimmedIp);
            };

            this.isIpv6AddressValid = function (ipAddress) {
              var acceptZoneIndex = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
              ...
              return !_this.equalIpv6Addresses(addr, IpParserService.IPV6_DEFAULT_UNICAST_ROUTE_ADDRESS) && !_this.equalIpv6Addresses(addr, IpParserService.IPV6_LOOPBACK_ADDRESS);
            };
            /**
            ...
            this.isSameIpv4Subnet = function (ipAddressFirst, ipAddressSecond, subnetMask) {
              if (!_this.isIpv4AddressValid(ipAddressFirst) || !_this.isIpv4AddressValid(ipAddressSecond) || !_this.isSubnetMaskValid(subnetMask)) {
                return false;
              }

              var splitIpFirst = ipAddressFirst.split(IpParserService.IPV4_SEPARATOR);
              var splitIpSecond = ipAddressSecond.split(IpParserService.IPV4_SEPARATOR);
              var splitSubnet = _this.isCidr(subnetMask) ? _this.parseCidrToIpv4(+subnetMask).split(IpParserService.IPV4_SEPARATOR) : subnetMask.split(IpParserService.IPV4_SEPARATOR); // enable bitwise operations
            ...
            this.isSameIpv6Subnet = function (ipAddressFirst, ipAddressSecond, prefix) {
            ...
            this.isAddressValid = function (ipAddress) {
              return _this.isIpv6AddressValid(ipAddress) || _this.isIpv4AddressValid(ipAddress);
            };

            this.expandIpv6Address = function (ipAddress) {
              if (!ipAddress || !_this.isIpv4AddressValid(ipAddress) && !_this.isIpv6AddressValid(ipAddress)) {
                return ipAddress;
              }

              var parsedAddress = Object(ipaddr_js__WEBPACK_IMPORTED_MODULE_0__["parse"])(ipAddress);
              return typeof parsedAddress.toNormalizedString !== "undefined" ? parsedAddress.toNormalizedString() : ipAddress;
            };

            this.validIpV4Address = function (ipAddress) {
              var splitIpAddress = ipAddress.split(IpParserService.IPV4_SEPARATOR);

              if (splitIpAddress.length !== 4) {
                return false;
              }

              return splitIpAddress.every(function (item) {
                return +item >= 0 && +item <= 255;
              });
            };

            this.isSubnetForIpValid = function (ipString) {
              var ipSections = ipString.split("/");
              var isIPv4 = Object(ipaddr_js__WEBPACK_IMPORTED_MODULE_0__["parse"])(ipSections[0]) instanceof ipaddr_js__WEBPACK_IMPORTED_MODULE_0__["IPv4"]; // IPv4 is 32 bits, each part is a 8 bits number;
              // IPv6 is 128 bits, each part is 16 bits

              var ipAddressDigitsCount = isIPv4 ? 8 : 16;
              var subnet = +ipSections[1];
              ...
              if (isIPv4 && !_this.isSubnetValid(subnet) || !isIPv4 && !_this.isSubnetPrefixValid(subnet.toString())) {
                return false;
              }

              var highDigits = subnet % ipAddressDigitsCount;
              var validationStartPosition = Math.floor(subnet / ipAddressDigitsCount);

              var ipNumbers = _this.parseIpToNum(ipSections[0]);

              if (ipNumbers.length === 0 || ipNumbers.find(function (num) {
                return isNaN(num);
              })) {
                return false;
              ...
              if (ipNumbers.find(function (num, index) {
                return num !== 0 && index > validationStartPosition;
              })) {
                return false;
              ...
              var subnetValidation = 0;

              for (var i = 1; i <= highDigits; i++) {
            ...
            this.parseIpToNum = function (ipStr) {
              var ip = Object(ipaddr_js__WEBPACK_IMPORTED_MODULE_0__["parse"])(ipStr);
            ...
            this.isSubnetPrefixValid = function (prefix) {
              return +prefix >= 1 && +prefix <= 128;
            };

            this.isSubnetValid = function (subnet) {
              return subnet >= 1 && subnet <= 32;
            };

            this.isSubnetMaskValid = function (subnetMask) {
              if (!subnetMask) {
                return false;
              ...
              var maskBytes = [];
              subnetMask.split(IpParserService.IPV4_SEPARATOR).forEach(function (octet) {
                if (!!octet) {
              ...
              for (var _i = 0, _maskBytes = maskBytes; _i < _maskBytes.length; _i++) {
                var maskByte = _maskBytes[_i];
            ...
            this.equalIpv6Addresses = function (addr1, addr2) {
              return _this.getNormalizedIpv6Address(addr1) === _this.getNormalizedIpv6Address(addr2);
            };
            /**
            ...
            this.getNormalizedIpv6Address = function (ipAddress) {
              var addr = Object(ipaddr_js__WEBPACK_IMPORTED_MODULE_0__["parse"])(ipAddress);
            ...
            this.parse = function (addressString) {
              var addressAndPort = _this.detectParts(addressString);

              var portResult = _this.parsePort(addressAndPort.port);

              var addressResult = _this.parseAddress(addressAndPort.address);

              if (portResult.valid && addressResult.valid) {
                return _this.buildIpAddress(addressResult.address, portResult.port);
              } else {
                return null;
              }
            };

            this.detectParts = function (addressString) {
              var parts = addressString.split(":");

              if (parts.length > 2) {
                return _this.detectIPv6Parts(addressString);
              } else {
                return {
                  address: parts[0],
                  port: parts[1]
            ...
            this.detectIPv6Parts = function (address) {
              var match = IpParserService.IPV6_BRACKETED_NOTATION_REGEX.exec(address);
            ...
            this.buildIpAddress = function (address, port) {
              return {
                address: address,
                port: port
              };
          ...
          _createClass(IpParserService, [{
            key: "isCidr",
            value: function isCidr(mask) {
              return !isNaN(+mask);
            }
            /**
            ...
            key: "parseCidrToIpv4",
            value: function parseCidrToIpv4(cidr) {
              if (!this.isSubnetMaskValid(String(cidr))) {
                return "";
              }

              var IP_ADDRESS_DIGITS_COUNT = 8;
              var subnet = [];

              for (var i = 0; i < Math.floor(cidr / IP_ADDRESS_DIGITS_COUNT); i++) {
            ...
            key: "isLinkLocalIpv6Address",
            value: function isLinkLocalIpv6Address(address) {
              var prefixLength = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 64;

              if (!address || prefixLength !== IpParserService.IPV6_LINK_LOCAL_PREFIX_LENGTH || address.length < 4) {
                return false;
              }

              var addressBegining = parseInt(address.substr(0, 4), 16);
        ...
        IpParserService.MAX_PORT = Math.pow(2, 16); // capture groups are: [fullText, address, portWithColon, port]
