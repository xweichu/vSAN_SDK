(function () {
  ...
  (window["webpackJsonp"] = window["webpackJsonp"] || []).push([[6], {
    ...
    "3For":
    /*!****************************************************************************!*\
      !*** ./src/app/vsan/common/component/chart/performance/perf-chart-util.ts ***!
      \****************************************************************************/

    /*! exports provided: PerfChartUtil, PerfChartDataConstants */

    /***/
    function For(module, __webpack_exports__, __webpack_require__) {
      "use strict";
      ...
      __webpack_require__.d(__webpack_exports__, "PerfChartUtil", function () {
        return PerfChartUtil;
      });
      /* harmony export (binding) */


      __webpack_require__.d(__webpack_exports__, "PerfChartDataConstants", function () {
        return PerfChartDataConstants;
      });
      /* harmony import */
      ...
      var _component_chart_model_metric_comparisson_type_data__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(
      /*! @component/chart/model/metric-comparisson-type.data */
      "w4Wc");
      /* harmony import */


      var _generated_perf_graph_threshold__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(
      /*! @generated/perf-graph-threshold */
      "/EAF");
      /* harmony import */


      var _generated_perf_graph_threshold_direction__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(
      ...
      var _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(
      /*! ../../../../performance/model/perf-monitor-view-type.model */
      "NvCx");
      /* harmony import */


      var _performance_view_perf_vm_mode_selector_perf_vm_mode__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(
      /*! ../../../../performance/view/perf-vm-mode-selector/perf-vm-mode */
      "FAE6");
      /* harmony import */


      var _perf_graph__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(
      /*! ./perf-graph */
      "lXqx");
      /* Copyright 2017-2022 VMware, Inc. All rights reserved. -- VMware Confidential */


      var PerfChartUtil = /*@__PURE__*/function () {
        var PerfChartUtil = /*#__PURE__*/function () {
          function PerfChartUtil() {
            _classCallCheck(this, PerfChartUtil);
          }

          _createClass(PerfChartUtil, null, [{
            key: "getConsolidatedChartOptions",
            value: function getConsolidatedChartOptions(config, afterSetExtremesXAxisCallback, customizableThreshold, onMouseOver, onMouseOut) {
              var series = [];
              config.consolidatedMetrics.forEach(function (entry) {
                if (entry.isLegend) {
                  // Only use black color for legend item which are displayed below the chart
                  entry[PerfChartUtil.LEGEND_COLOR] = PerfChartUtil.BLACK_METRIC_LEGEND;
                  entry[PerfChartUtil.LEGEND_ID] = entry.key;
                } else {
                  entry[PerfChartUtil.LEGEND_COLOR] = config.entityIdentifierToColor[entry.entityIdentifier.id]; // Map to the legend item so that clicking on the legend entry toggles the chart lines

                  entry[PerfChartUtil.LEGEND_LINKED_TO] = entry.key;
                }

                entry[PerfChartUtil.LEGEND_SHOW_IN_LEGEND] = entry.isLegend;
                entry[PerfChartUtil.LEGEND_DASH_STYLE] = entry.lineStyle;
                series.push(entry);
              });
              var consolidatedChartOptions = PerfChartUtil.getBasicChartOptions(config.graph, this.getShortestSeriesLength(series), config.legendPosition);
              consolidatedChartOptions.series = series;
              PerfChartUtil.addSeriesEvent(consolidatedChartOptions.plotOptions.series.point, onMouseOver, onMouseOut);
              consolidatedChartOptions.xAxis = PerfChartUtil.getXAxisOptions(config.graph, config.firstEntityPerfData && config.firstEntityPerfData.timeStamps, config.looseXAxisLabel, afterSetExtremesXAxisCallback);
              consolidatedChartOptions.yAxis = PerfChartUtil.getYAxisOptions(config.graph, config.firstEntityPerfData, series, customizableThreshold);
              consolidatedChartOptions.tooltip = {
                useHTML: true,
                headerFormat: PerfChartUtil.getConsolidatedHeaderFormat(config.graph),
                pointFormatter: PerfChartUtil.getConsolidatedPointFormatter(config.graph),

                /*
                * Show tooltip only for current hovered metric, not for all of them
                * */
                shared: false
              };
              return consolidatedChartOptions;
            }
            /**
             * return the chart options according to the graph data
             */

          }, {
            key: "getChartOptions",
            value: function getChartOptions(entityRefId, graphConfig, metrics) {
              var index = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;
              var looseXAxisLabel = arguments.length > 4 ? arguments[4] : undefined;
              var legendPostion = arguments.length > 5 ? arguments[5] : undefined;
              var afterSetExtremesXAxisCallback = arguments.length > 6 ? arguments[6] : undefined;
              var customizableThreshold = arguments.length > 7 ? arguments[7] : undefined;
              var keepPlotLineOnClick = arguments.length > 8 ? arguments[8] : undefined;
              var onPlotLineClicked = arguments.length > 9 ? arguments[9] : undefined;
              var actionLink = arguments.length > 10 ? arguments[10] : undefined;
              var seriesMouseOverCallback = arguments.length > 11 ? arguments[11] : undefined;
              var seriesMouseOutCallback = arguments.length > 12 ? arguments[12] : undefined;
              var series = PerfChartUtil.getChartSeries(graphConfig, metrics);
              var basicOptions = PerfChartUtil.getBasicChartOptions(graphConfig, this.getShortestSeriesLength(series), legendPostion, keepPlotLineOnClick, onPlotLineClicked);
              basicOptions.xAxis = PerfChartUtil.getXAxisOptions(graphConfig, metrics && metrics.timeStamps, looseXAxisLabel, afterSetExtremesXAxisCallback);
              var additionalGraph = graphConfig.secondGraph;
              /*
              * Check if there is secondaryGraph in graph object, if so then build it's config and display it
              */

              if (additionalGraph != null) {
                basicOptions = PerfChartUtil.getSecondGraphOptions(basicOptions, graphConfig, metrics, series, additionalGraph, looseXAxisLabel, afterSetExtremesXAxisCallback, customizableThreshold);
              } else {
                basicOptions.series = series;
                basicOptions.yAxis = PerfChartUtil.getYAxisOptions(graphConfig, metrics, series, customizableThreshold);
              }

              this.addSeriesEvent(basicOptions.plotOptions.series.point, seriesMouseOverCallback, seriesMouseOutCallback);
              basicOptions.tooltip = {
                // keep the tooltip open if it contains a link
                stickOnContact: !!actionLink,
                // html formatters
                headerFormat: graphConfig.isHistogram ? PerfChartUtil.getHistogramHeaderFormat(graphConfig) : null,
                pointFormatter: graphConfig.isHistogram ? PerfChartUtil.getHistogramPointFormatter(graphConfig) : null,
                footerFormat: graphConfig.isHistogram ? PerfChartUtil.getHistogramFooterFormat(graphConfig) : null,
                // non html formatter
                formatter: graphConfig.isHistogram ? null : PerfChartUtil.getSeparatedTooltipFormatter(graphConfig, metrics, actionLink),
                shared: true,
                useHTML: true,
                // Currently stickOnContact and outside can't work together due to the bug
                // https://github.com/highcharts/highcharts/issues/14434
                // Once the issue got resolved and highcharts version updated; TODO [mavrodievd] Set "outside: true"
                outside: !actionLink
              }; // Disable zoom feature for histogram charts

              if (graphConfig.isHistogram) {
                basicOptions.chart.zoomType = {
                  enabled: false
                };
              }
            ...
            key: "getShortestSeriesLength",
            value: function getShortestSeriesLength(series) {
              var shortestSeriesLength = Number.MAX_VALUE; // compare against the length of all not nullable values in the series

              series === null || series === void 0 ? void 0 : series.forEach(function (s) {
                var _a;

                var seriesLength = (_a = s === null || s === void 0 ? void 0 : s.data) === null || _a === void 0 ? void 0 : _a.filter(function (data) {
                  return !_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].isUnset(data);
                }).length;

                if (seriesLength) {
                  shortestSeriesLength = Math.min(shortestSeriesLength, seriesLength);
                }
              });
              return shortestSeriesLength;
            }
          }, {
            key: "getHistogramHeaderFormat",
            value: function getHistogramHeaderFormat(graphConfig) {
              var hasCategories = PerfChartUtil.getHistogramChartCategories(graphConfig) !== null;

              if (hasCategories) {
                return "<table>";
              } else {
                return "<b>{point.key}</b>";
              }
            }
          }, {
            key: "getConsolidatedHeaderFormat",
            value: function getConsolidatedHeaderFormat(graphConfig) {
              return "<div>{point.key}</div>";
            }
          }, {
            key: "getHistogramPointFormatter",
            value: function getHistogramPointFormatter(graphConfig) {
              var hasCategories = PerfChartUtil.getHistogramChartCategories(graphConfig) !== null;
              return function (tooltip) {
                var value = _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.perf.chart.label.unit.percentage", _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].localizeNumber(PerfChartUtil.getAsPercentages(this.y, graphConfig.unit)));

                if (hasCategories) {
                  return "<tr>\n                     <td>\n                        <svg class=\"tooltip-color-flag\" style=\"fill:" + this.series.color + "\">\n                           <rect x=\"0\" y=\"4\" width=\"25\" height=\"6\"/>\n                         </svg>\n                         ".concat(this.series.name, ":&nbsp;\n                     </td>\n                     <td style='padding:0'><b>").concat(value, "</b></td>\n                   </tr>");
                } else {
            ...
            key: "getConsolidatedPointFormatter",
            value: function getConsolidatedPointFormatter(graph) {
              return function (tooltip) {
                var _this2 = this;

                if (this.isNull) {
                  return _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.na.label");
                }

                var selectedSeriesTooltipContent = "<table>";
                var selectedSeries = this.series;
                /* Set tooltip content for hovered metric */

                var entityName = PerfChartUtil.escapeAngledBrackets(selectedSeries.userOptions.entityIdentifier.name);
                var metricName = PerfChartUtil.escapeAngledBrackets(selectedSeries.userOptions.metricName);
                var value = PerfChartUtil.formatYAxisLabel(selectedSeries.processedYData[this.index], graph);
                var relatedPointsTooltipContent = "";
                var relatedSeries = [];
                /*
                   Traverse all metrics and find such that are visually overlapped by the currently hovered metric
                   and cannot be hovered with the mouse
                   Add them to relatedPointsTooltipContent so they can be seen
                */

                this.series.chart.series.forEach(function (seriesEntry) {
                  if (selectedSeries === seriesEntry || selectedSeries.name === seriesEntry.name || seriesEntry.userOptions.isLegend || !seriesEntry.visible || relatedSeries.includes(seriesEntry.name)) {
                    return;
                  }

                  var pointY = seriesEntry.processedYData[_this2.index];
                  var diffY = Math.abs(seriesEntry.processedYData[_this2.index] - _this2.y);
                  /*
                   Check if current series is 0.1 max away from hovered point
                   */

                  if (diffY <= 0.1) {
                    var valueY = pointY != null ? PerfChartUtil.formatYAxisLabel(pointY, graph) : _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.na.label");
                    var entityNameSeriesEntry = PerfChartUtil.escapeAngledBrackets(seriesEntry.userOptions.entityIdentifier.name);
                    var metricNameSeriesEntry = PerfChartUtil.escapeAngledBrackets(seriesEntry.userOptions.metricName);
                    relatedPointsTooltipContent += PerfChartUtil.getSeriesTooltipContent([], entityNameSeriesEntry, metricNameSeriesEntry, valueY, seriesEntry.color);
                    relatedSeries.push(seriesEntry.name);
                  }
                }); // do not add bottom padding when we have only one series

                selectedSeriesTooltipContent += PerfChartUtil.getSeriesTooltipContent(relatedSeries, entityName, metricName, value, selectedSeries.color);
                selectedSeriesTooltipContent += relatedPointsTooltipContent;
                selectedSeriesTooltipContent += "</table>";
                return selectedSeriesTooltipContent;
              };
            }
          }, {
            key: "getSeriesTooltipContent",
            value: function getSeriesTooltipContent(relatedSeries, entityName, metricName, value, color) {
              return "<tr>\n                  <td ".concat(relatedSeries.length > 0 ? "class=\"tooltip-padding-bottom\"" : "", ">\n                     <svg class=\"tooltip-color-flag\" style=\"fill:").concat(color, "\">\n                        <rect x=\"0\" y=\"4\" width=\"25\" height=\"6\"/>\n                     </svg>\n                     <b>").concat(entityName, "</b> ").concat(metricName, ":</td>\n                  <td ").concat(relatedSeries.length > 0 ? "class=\"tooltip-padding-bottom tooltip-value\"" : "class=\"tooltip-value\"", ">").concat(value, "</td>\n               </tr>");
            }
          }, {
            key: "getHistogramFooterFormat",
            value: function getHistogramFooterFormat(graphConfig) {
              var hasCategories = PerfChartUtil.getHistogramChartCategories(graphConfig) != null;
            ...
            key: "escapeAngledBrackets",
            value: function escapeAngledBrackets(text) {
              return String(text).replace(/</g, "&lt;").replace(/>/g, "&gt;");
            }
          }, {
            key: "getSecondGraphOptions",
            value: function getSecondGraphOptions(options, primaryGraph, primaryGraphMetrics, primaryGraphSeries, secondaryGraph, looseXAxisLabel, afterSetExtremesXAxisCallback, customizableThreshold) {
              // Get additional graph series
              var secondYAxisSeries = PerfChartUtil.getChartSeries(secondaryGraph, primaryGraphMetrics); // Set yAxis index for secondary graph series

              secondYAxisSeries = secondYAxisSeries.map(function (seriesItem) {
                // Set yAxis index
                seriesItem.yAxis = 1;
                return seriesItem;
              }); // Set yAxis index for primary graph series

              primaryGraphSeries = primaryGraphSeries.map(function (seriesItem) {
                // Set yAxis index
                seriesItem.yAxis = 0;
                return seriesItem;
              }); // Build primary yAxis

              var primaryYAxis = PerfChartUtil.getYAxisOptions(primaryGraph, primaryGraphMetrics, primaryGraphSeries, customizableThreshold);
              primaryYAxis.title.text = primaryGraph.label; // Build secondary yAxis

              var secondaryYAxis = PerfChartUtil.getYAxisOptions(secondaryGraph, primaryGraphMetrics, secondYAxisSeries, customizableThreshold); // Set opposite side for second graph in order to display second graph scala on right

              secondaryYAxis.opposite = true;
              secondaryYAxis.gridLineWidth = 0;
              secondaryYAxis.title.text = secondaryGraph.label; // Set promary and secondary y axis to chart options

              options.yAxis = [primaryYAxis, secondaryYAxis]; // Build warning areas based on graph type and series

              var warningAreaPolygons = PerfChartUtil.getWarningAreas(secondaryGraph, secondYAxisSeries); // Set warning areas series, primary graph series and secondary graph series

              if (warningAreaPolygons != null) {
                options.series = [].concat(_toConsumableArray(warningAreaPolygons.polygonSeries), _toConsumableArray(primaryGraphSeries), _toConsumableArray(secondYAxisSeries)); // Add label for warning areas

                options.series.push(warningAreaPolygons.legendLabel);
              } else {
                options.series = [].concat(_toConsumableArray(primaryGraphSeries), _toConsumableArray(secondYAxisSeries));
              }

              return options;
            }
            /*
            * Check if graph must have warning areas for some of it's metrics
            */

          }, {
            key: "getWarningAreas",
            value: function getWarningAreas(graph, series) {
              var chartMaximumValue = Number.MAX_SAFE_INTEGER;
              var spec;
              var legendLabel;
              /*
              ...
              */
              ...
              if (graph.id === "com.vmware.vsan.perf.graph.virtual-disk.normalized-iops") {
                var iopsLimitMetricName = graph.metrics.find(function (x) {
                  return x.key === "iopsLimit";
                }).label;
                var normalizedIopsMetricName = graph.metrics.find(function (x) {
                  return x.key === "NIOPS";
                }).label;
                var labelId = "iops-limit-latency";
                spec = {
                  labelId: labelId,
                  firstMetric: series.find(function (x) {
                    return x.name === iopsLimitMetricName;
                  }),
                  secondMetric: series.find(function (x) {
                    return x.name === normalizedIopsMetricName;
                  }),
                  comparator: _component_chart_model_metric_comparisson_type_data__WEBPACK_IMPORTED_MODULE_1__["MetricComparissonType"].EQUAL_OR_LESS,
                  max: chartMaximumValue
                };
                legendLabel = {
                  name: _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.monitor.vm.performance.virtualDisk.indicate.iopLimit.latency"),
                  id: labelId,
                  color: "yellow"
                };
              }

              var polygonSeries = PerfChartUtil.buildAreaPolygons(spec);

              if (polygonSeries == null || polygonSeries.length === 0) {
                return null;
              }
            ...
            /*
            * Fill some areas of the chart with color by given expression < > =
            */

          }, {
            key: "buildAreaPolygons",
            value: function buildAreaPolygons(spec) {
              var polygons = [];

              if (!spec.firstMetric && !spec.secondMetric) {
                return polygons;
              }
              ...
              var rightTopPoint;
              var leftTopPoint;
              spec.firstMetric.data.push(undefined);
              spec.secondMetric.data.push(undefined); // Build polygons with coordinates based on series values and operator

              spec.firstMetric.data.forEach(function (val, index) {
                var comparisonResult = false;

                switch (spec.comparator) {
                  case _component_chart_model_metric_comparisson_type_data__WEBPACK_IMPORTED_MODULE_1__["MetricComparissonType"].GREATER:
                    comparisonResult = val > spec.secondMetric.data[index];
                    break;

                  case _component_chart_model_metric_comparisson_type_data__WEBPACK_IMPORTED_MODULE_1__["MetricComparissonType"].EQUAL_OR_GREATER:
                    comparisonResult = val >= spec.secondMetric.data[index];
                    break;

                  case _component_chart_model_metric_comparisson_type_data__WEBPACK_IMPORTED_MODULE_1__["MetricComparissonType"].LESS:
                    comparisonResult = val < spec.secondMetric.data[index];
                    break;

                  case _component_chart_model_metric_comparisson_type_data__WEBPACK_IMPORTED_MODULE_1__["MetricComparissonType"].EQUAL_OR_LESS:
                    comparisonResult = val <= spec.secondMetric.data[index];
                    break;

                  case _component_chart_model_metric_comparisson_type_data__WEBPACK_IMPORTED_MODULE_1__["MetricComparissonType"].EQUAL:
                    comparisonResult = val === spec.secondMetric.data[index];
                    break;
                }

                if (val == null || spec.secondMetric.data[index] == null) {
                  comparisonResult = false;
                } // Check if comparison between first and second series for given index is satisfied


                if (comparisonResult && index < spec.firstMetric.data.length - 1) {
                  // If so, then check if polygon starting point is already set.
                  // If it is then we are in the middle of polygon construction and should not do anything
                  // But if not then set starting point of the polygon
                  ...
                  var endIndex = comparisonResult ? index : index - 1;
                  rightBottomPoint = {
                    x: endIndex,
                    y: 0
                  };
                  ...
                  polygons.push({
                    name: PerfChartUtil.WARNING_AREA_NAME,
                    type: "area",
                    color: "rgba(255,255,153,0.99)",
                    showInLegend: false,
                    linkedTo: spec.labelId,
                    data: [[leftBottomPoint.x, leftBottomPoint.y], [rightBottomPoint.x, rightBottomPoint.y], [rightTopPoint.x, rightTopPoint.y], [leftTopPoint.x, leftTopPoint.y]]
                  }); // Start building new polygon by setting leftBottomPoint to null because current one is added.

                  leftBottomPoint = null;
                }
              });
              spec.firstMetric.data.pop();
              spec.secondMetric.data.pop();
              return polygons;
            }
          }, {
            key: "getSeparatedTooltipFormatter",
            value: function getSeparatedTooltipFormatter(graphConfig, metrics, actionLink) {
              return function (tooltip) {
                var currentPage = tooltip.chart.legend.currentPage;
                var itemsPerPage = PerfChartUtil.legendItemsPerPage(tooltip);
                var s = "<div>".concat(this.x, "</div><table>");
                /**
                ...
                this.points.forEach(function (point, index) {
                  ...
                  if (point.series.name === PerfChartUtil.WARNING_AREA_NAME) {
                    return;
                  }

                  if (!PerfChartUtil.isInCurrentPage(index + 1, currentPage, itemsPerPage)) {
                    return;
                  } // Check if current point is part of series from second graph,
                  // if so the get data from there


                  var currentGraphConfig;

                  if (graphConfig.secondGraph != null && graphConfig.secondGraph.metrics.find(function (x) {
                    return x.label === point.series.name;
                  })) {
                    currentGraphConfig = graphConfig.secondGraph;
                  } else {
                    currentGraphConfig = graphConfig;
                  }

                  s += "<tr>\n                     <td>\n                        <svg class=\"tooltip-color-flag\" style=\"fill: ".concat(point.series.color, "\">\n                           <rect x=\"0\" y=\"4\" width=\"25\" height=\"6\"/>\n                        </svg>").concat(PerfChartUtil.escapeAngledBrackets(point.series.name), ":\n                     </td>\n                     <td class=\"tooltip-value\">\n                        ").concat(PerfChartUtil.formatYAxisLabel(point.y, currentGraphConfig, false, (_a = point.series.userOptions) === null || _a === void 0 ? void 0 : _a.metricKey), "\n                     </td>\n                  </tr>");
                });

                if (actionLink) {
                  var timePoint = metrics.timeStamps[this.points[0].point.index];
                  s += "<td>\n                     <a class=\"tooltip-action btn-link btn-sm btn\"\n                      href=\"javascript://\"\n                      graphId=\"".concat(graphConfig.id, "\"\n                      timePoint=\"").concat(timePoint, "\">\n                      ").concat(actionLink, "</a>\n                  </td></table>");
                } else {
                  s += "</table>";
                }
            ...
            key: "isInCurrentPage",
            value: function isInCurrentPage(index, currentPage, itemsPerPage) {
              if (itemsPerPage === -1) {
                // If itemsPerPage = -1, there isn't pagination.
                return true;
              } // Check if the current item belongs to the current page
              // Also we want to include the first item from the NEXT page, because it's shown in the chart legend.


              return index > (currentPage - 1) * itemsPerPage && index <= currentPage * itemsPerPage + 1;
            }
            /**
             * Get the legend items' count per page
             */

          }, {
            key: "legendItemsPerPage",
            value: function legendItemsPerPage(tooltip) {
              var legend = tooltip.chart.legend;
              return !_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].isEmpty(legend.pages) ? Math.floor(legend.clipHeight / legend.itemHeight) : -1;
            }
          }, {
            key: "getBasicChartOptions",
            value: function getBasicChartOptions(graphConfig, shortestSeriesLength, legendPosition) {
              var keepPlotLineOnClick = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
              var onPlotLineClicked = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
              return {
                // Todo akolov: discuss with @lpetkov and use ClarityColors
                colors: ["#7b71c9", "#188fb2", "#9cc617", "#448230", "#5E9CFF", "#32428A", "#C1C1C1"],
                title: {
                  text: undefined
                },
                exporting: {
                ...
                chart: {
                  animation: false,
                  type: graphConfig.isHistogram ? "column" : "line",
                  zoomType: "x",
                  reflow: true,
                  resetZoomButton: {
                    theme: {
                  ...
                  height: PerfChartUtil.CHART_HEIGHT
                },

                /**
                 * legend section can only be at the bottom or right side, it's at bottom if not set.
                 */
                legend: this.getLegend(graphConfig, legendPosition),
                plotOptions: {
                  series: {
                    marker: {
                      // If there are series with only 1 point, then show pointers.
                      enabled: shortestSeriesLength === 1
                    },
                    lineWidth: 1.5,
                    turboThreshold: 0,
                    point: {
                      events: {
                        click: function click(e) {
                          var _this = this; // "this" points to the hovered point object


                          if (keepPlotLineOnClick) {
                            highcharts__WEBPACK_IMPORTED_MODULE_9__["charts"].forEach(function (chart) {
                              PerfChartUtil.addPlotLine(chart, _this.x);
                            });
                          }

                          if (onPlotLineClicked) {
                            onPlotLineClicked(_this);
                          }
                        }
                      }
                    }
            ...
            key: "addPlotLine",
            value: function addPlotLine(chart, valueIndex) {
              if (chart && chart.container && !_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].isEmpty(chart.series) && chart.series[0].xAxis) {
                var plotLineId = chart.container.id + "_plotline";
                var color = chart.series[0].xAxis.crosshair.color;
                var crossHairColor = !color ? "grey" : color;
                chart.series[0].xAxis.removePlotLine(plotLineId);
            ...
            key: "addSeriesEvent",
            value: function addSeriesEvent(plotOptionsSeries, mouseOverEvent, mouseOutEvent) {
              if (!mouseOverEvent && !mouseOutEvent) {
                return;
              }

              if (!plotOptionsSeries.events) {
                plotOptionsSeries.events = {};
              }

              if (mouseOverEvent) {
                plotOptionsSeries.events.mouseOver = mouseOverEvent;
              }

              if (mouseOutEvent) {
                plotOptionsSeries.events.mouseOut = mouseOutEvent;
              }
            }
          }, {
            key: "getYAxisOptions",
            value: function getYAxisOptions(graphConfig, metrics, series, customizableThreshold) {
              var max = PerfChartUtil.getYAxisMaxVal(graphConfig, series, customizableThreshold);
              return {
                title: {
                  text: undefined
                },
                labels: {
                  formatter: function formatter() {
                    return PerfChartUtil.formatYAxisLabel(this.value, graphConfig, !!customizableThreshold);
                  }
                },
                plotBands: PerfChartUtil.getThresholdBands(PerfChartUtil.getThreshold(graphConfig, metrics, customizableThreshold), max),
                max: max,
                tickInterval: max / 2
              };
            }
            ...
            key: "getLegend",
            value: function getLegend(graph, legendPosition) {
              if (graph.isHistogram) {
                var hasCategories = PerfChartUtil.getHistogramChartCategories(graph) !== null;
                return hasCategories ? PerfChartUtil.BOTTOM_LEGEND_OPTION : PerfChartUtil.NO_LEGEND_OPTION;
              }

              return legendPosition === PerfChartUtil.LEGEND_RIGHT ? PerfChartUtil.RIGHT_LEGEND_OPTION : PerfChartUtil.BOTTOM_LEGEND_OPTION;
            }
            /**
             * Get the threshold object by searching into two places:
             *  * graph definition
             *  * metrics data
             *  * customized thresholds returned by VsanDiagnosticsSystem.getThresholds API
             */

          }, {
            key: "getThreshold",
            value: function getThreshold(graph, metrics, customizableThreshold) {
              if (customizableThreshold) {
                return customizableThreshold;
              } // If the threshold has been defined in the graph definition, use it


              if (graph && graph.threshold) {
                return graph.threshold;
              } // Otherwise - search in the metrics


              return PerfChartUtil.getThresholdFromMetrics(graph, metrics);
            }
          }, {
            key: "getCustomizableThreshold",
            value: function getCustomizableThreshold(graph, customizableThresholds) {
              var threshold;

              if (!_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].isEmpty(customizableThresholds)) {
                // TODO: bbambalo We expect only one metric per graph, because there is no way to show multiple
                // metrics and thresholds for a graph. Update to use directly the graph.metrics[0] when backend starts return
                // only one metric
                var _iterator = _createForOfIteratorHelper(graph.metrics),
                    _step;

                try {
                  var _loop = function _loop() {
                    var metric = _step.value;
                    threshold = customizableThresholds.find(function (t) {
                      return metric.key === t.metric;
                    });
                    return "break";
                  };
            ...
            key: "toPercentageThreshold",
            value: function toPercentageThreshold(thresholdInPermille) {
              if (!thresholdInPermille) {
                return null;
              }

              var thresholdInPercentage = _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].deepClone(thresholdInPermille);

              thresholdInPercentage.yellow = PerfChartUtil.toPercentages(thresholdInPermille.yellow);
              thresholdInPercentage.red = PerfChartUtil.toPercentages(thresholdInPermille.red);
              return thresholdInPercentage;
            }
          }, {
            key: "toPermilleThreshold",
            value: function toPermilleThreshold(thresholdInPercentage) {
              if (!thresholdInPercentage) {
                return null;
              }

              var thresholdInPermille = _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].deepClone(thresholdInPercentage);

              thresholdInPermille.yellow = PerfChartUtil.toPermilles(thresholdInPercentage.yellow);
              thresholdInPermille.red = PerfChartUtil.toPermilles(thresholdInPercentage.red);
              return thresholdInPermille;
            }
          }, {
            key: "getAsPercentages",
            value: function getAsPercentages(value, valueUnit) {
              return valueUnit === PerfChartUtil.PERMILLE_TYPE ? PerfChartUtil.toPercentages(value) : value;
            }
          }, {
            key: "getThresholdFromMetrics",
            value: function getThresholdFromMetrics(graph, metrics) {
              if (!metrics || _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].isEmpty(metrics.metricsSeries)) {
                return null;
              }

              var metricIds = new Set(graph.metrics.map(function (metric) {
                return metric.key;
              }));
              var threshold = null;

              var _iterator2 = _createForOfIteratorHelper(metrics.metricsSeries),
                  _step2;
              ...
              try {
                for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
                  ...
                  if (metricIds.has(metric.key) && metric.threshold) {
                    threshold = _generated_perf_graph_threshold__WEBPACK_IMPORTED_MODULE_2__["PerfGraphThreshold"].Factory.create(graph.id, metric.key, metric.threshold.yellow, metric.threshold.red, metric.threshold.direction);
                    break;
                  }
                }
              } catch (err) {
            ...
            key: "getThresholdBands",
            value: function getThresholdBands(threshold, max) {
              var bands = [];
              ...
              if (threshold.direction === _generated_perf_graph_threshold_direction__WEBPACK_IMPORTED_MODULE_3__["PerfGraphThresholdDirection"].upper) {
                if (threshold.red) {
                  bands.push(PerfChartUtil.createBand(PerfChartUtil.RED_THRESHOLD_COLOR, threshold.red, max));
                }
                ...
                if (threshold.yellow) {
                  ...
                  if (threshold.red) {
                    yellowMax = threshold.red;
                  }

                  bands.push(PerfChartUtil.createBand(PerfChartUtil.YELLOW_THRESHOLD_COLOR, threshold.yellow, yellowMax));
                }
              } else if (threshold.direction === _generated_perf_graph_threshold_direction__WEBPACK_IMPORTED_MODULE_3__["PerfGraphThresholdDirection"].lower) {
                if (threshold.red) {
                  bands.push(PerfChartUtil.createBand(PerfChartUtil.RED_THRESHOLD_COLOR, 0, threshold.red));
                }
                ...
                if (threshold.yellow) {
                  ...
                  if (threshold.red) {
                    yellowMin = threshold.red;
                  }

                  bands.push(PerfChartUtil.createBand(PerfChartUtil.YELLOW_THRESHOLD_COLOR, yellowMin, threshold.yellow));
                }
              } else {
                _util_logger__WEBPACK_IMPORTED_MODULE_6__["Logger"].warn("Unknown direction: " + threshold.direction);
              }

              return bands;
            }
            /**
             * Creates a band object that is used by the Chart engine to draw "red" and "yellow" sections
             */

          }, {
            key: "createBand",
            value: function createBand(color, from, to) {
            ...
            /**
             * format yAxis label, according to the graph.unit
             */

          }, {
            key: "formatYAxisLabel",
            value: function formatYAxisLabel(data, graph) {
              var hasCustomizableThreshold = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
              var metricKey = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;
              ...
              if (metricKey) {
                var metricUnit = (_a = graph.metrics.find(function (metric) {
                  return metric.key === metricKey;
                })) === null || _a === void 0 ? void 0 : _a.unit;
              ...
              switch (unit) {
                case PerfChartUtil.PERCENTAGE_TYPE:
                  // Digits info used to display up to 5 digits after the floating point without rounding.
                  var digitsInfo = ".0-2";
                  return new _angular_common__WEBPACK_IMPORTED_MODULE_0__["PercentPipe"](_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getLocaleForDecimal()).transform(data / 100, digitsInfo);

                case PerfChartUtil.RATE_BYTES_TYPE:
                  return _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.perf.chart.label.unit.persecond", [_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].formatFileSize(data)]);

                case PerfChartUtil.SIZE_BYTES_TYPE:
                  return _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].formatFileSize(data);

                case PerfChartUtil.TIME_S_TYPE:
                  return _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.perf.chart.label.unit.time_s", [_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].localizeNumber(data)]);

                case PerfChartUtil.TIME_MS_TYPE:
                  return _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.perf.chart.label.unit.latency", [_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].localizeNumber(data / 1000)]);

                case PerfChartUtil.PERMILLE_TYPE:
                  return _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.perf.chart.label.unit.percentage", [_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].localizeNumber(PerfChartUtil.toPercentages(data))]);

                case PerfChartUtil.MBYTES_TYPE:
                  return _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].formatFileSize(data * 1024 * 1024);

                default:
                  return _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].localizeNumber(data);
              }
            }
            /**
             * get the max value for yAxis
             * usually the max value comes from the chart data, except the max from data is 0.
             * in that case, the default max is given
             */

          }, {
            key: "getYAxisMaxVal",
            value: function getYAxisMaxVal(graph, series, customizableThresholds) {
              // Histogram charts and charts with customized thresholds which are in permilles exceed Y axis.
              // So, to scale the chart to support permille values, we need to convert Y axis values to permilles as well.
              if (this.isGraphInPermilles(graph, customizableThresholds)) {
                return PerfChartUtil.toPermilles(100);
              }

              if (series && series.length) {
                var flat = series.filter(function (item) {
                ...
                return max > 0 // Add 5% in addition to the max value, in order to allow hovering the max point.
                ? max + max * .05 : PerfChartUtil.getNoValMax(graph);
              }
            }
          }, {
            key: "isGraphInPermilles",
            value: function isGraphInPermilles(graph, customizableThresholds) {
              return (graph.isHistogram || customizableThresholds) && graph.unit === PerfChartUtil.PERMILLE_TYPE;
            }
            /**
             * get the default Max value for the yAxis if the max value from the series is 0
             */

          }, {
            key: "getNoValMax",
            value: function getNoValMax(graph) {
              switch (graph.unit) {
                case PerfChartUtil.PERCENTAGE_TYPE:
                  return 100;
            ...
            /**
             * format the xAxis labels. The xAxis labels are time strings in perf chart
             */

          }, {
            key: "getXAxisOptions",
            value: function getXAxisOptions(graphConfig, timeArray, looseXAxisLabel, afterSetExtremesXAxisCallback) {
              if (!timeArray || !timeArray.length) {
                return null;
              }

              var dateArr = timeArray.map(_util_vsan_dateteime_util__WEBPACK_IMPORTED_MODULE_7__["VsanDateTimeUtils"].convertDateToIso8601);
              var dateFormatter = new _angular_common__WEBPACK_IMPORTED_MODULE_0__["DatePipe"](_util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getLocaleForDecimal());
              var formatPattern = this.getDateFormatPattern(dateArr, dateFormatter);
              /**
               * Calculate the tickInterval to avoid the crowd labels on the x axis when the time range is big.
               * Usually the length of the timeArray is 12*n + 1 because backend saved data every 5m.
               * For small charts, we need bigger tickInterval.
               */

              var tickInterval = looseXAxisLabel ? Math.floor((dateArr.length - 1) / 2) : Math.floor((dateArr.length - 1) / 4);
              return {
                categories: graphConfig.isHistogram ? PerfChartUtil.getHistogramChartCategories(graphConfig) : dateArr.map(function (date) {
                  return dateFormatter.transform(date, formatPattern);
                }),
                type: graphConfig.isHistogram ? "category" : null,
                labels: graphConfig.isHistogram ? {
                  rotation: PerfChartUtil.HISTOGRAM_LABEL_ROTATION
                } : {},
                tickInterval: graphConfig.isHistogram ? null : tickInterval,
                crosshair: graphConfig.isHistogram ? null : {
                  width: 1
                },
                events: {
                  afterSetExtremes: afterSetExtremesXAxisCallback
                },
                gridLineWidth: 0.5
              };
            }
            /**
             * Gets the date format pattern for the chart timestamps.
             */

          }, {
            key: "getDateFormatPattern",
            value: function getDateFormatPattern(dates, dateFormatter) {
              var isSameDay = dateFormatter.transform(dates[0], PerfChartUtil.SHORT_DATE_PATTERN) === dateFormatter.transform(dates[dates.length - 1], PerfChartUtil.SHORT_DATE_PATTERN);
              var startFromToday = dateFormatter.transform(dates[0], PerfChartUtil.SHORT_DATE_PATTERN) === dateFormatter.transform(new Date(), PerfChartUtil.SHORT_DATE_PATTERN);
              var hasSeconds = dates === null || dates === void 0 ? void 0 : dates.some(function (date) {
                return date.getSeconds() > 0;
              });
              var formatPattern = hasSeconds ? PerfChartUtil.MEDIUM_TIME_PATTERN : PerfChartUtil.SHORT_TIME_PATTERN; // When the start time and end time are not on the same day or when the time does not start from today,
              // the X-axis date is displayed.

              if (!isSameDay || !startFromToday) {
                formatPattern = hasSeconds ? PerfChartUtil.SHORT_WITH_SECONDS_PATTERN : PerfChartUtil.SHORT_PATTERN;
              }

              return formatPattern;
            }
          }, {
            key: "getHistogramChartCategories",
            value: function getHistogramChartCategories(graphConfig) {
              var categories = new Set(graphConfig.metrics.map(function (metric) {
                return metric.group;
              }));
              return categories.size > 0 ? Array.from(categories) : null;
            }
            /**
             * build the chart series according to the graph config, the data is in the metrics
             *
             */

          }, {
            key: "getChartSeries",
            value: function getChartSeries(graphConfig, perfEntityMetrics) {
              if (!graphConfig || !graphConfig.metrics || !perfEntityMetrics) {
                return null;
              }
              ...
              if (graphConfig.isHistogram) {
                categories = PerfChartUtil.getHistogramChartCategories(graphConfig);

                if (categories !== null) {
                  PerfChartUtil.createDefaultHistogramSeries(chartSeries);
                }
              }

              graphConfig.metrics // filter the unnecessary graphs (the graphs which don't have series data)
              .filter(function (gMetric) {
                return perfEntityMetrics.metricsSeries.some(function (metricsData) {
                  return metricsData.key === gMetric.key;
                });
              }).forEach(function (gMetric) {
                // consider it as a chart line definition to better understand the logic
                var lineSeries = perfEntityMetrics.metricsSeries.filter(function (metricsData) {
                  return metricsData.key === gMetric.key;
                });
                lineSeries.forEach(function (line) {
                  var matched = line && line.values.length ? line.values : // return an array like [null, null, ..., null],
                  // the array must have same length with the timeStamps
                  // because highcharts throws exception if using empty array '[]'
                  perfEntityMetrics.timeStamps.map(function () {
                    return null;
                  });

                  if (graphConfig.isHistogram) {
                    PerfChartUtil.createHistogramSeries(graphConfig, gMetric, matched[0], categories, chartSeries);
                  } else {
                    var series = {
                      name: lineSeries.length === 1 ? gMetric.label : PerfChartUtil.getSeriesLabel(gMetric.label, line.subKey),
                      data: matched,
                      metricKey: gMetric.key
                    };
            ...
            key: "createHistogramSeries",
            value: function createHistogramSeries(graphConfig, gMetric, metricValue, categories, chartSeries) {
              if (categories) {
                // for histogram graph with categories, we have to create two series Read and Write and put the appropriate
                // metric regarding its prefix to the the corresponding series.
                // The index at the series must be the same as the index of the category at the categories array
                var group = graphConfig.metrics.find(function (metric) {
                  return metric.key === gMetric.key;
                }).group;
                var groupIndex = categories.findIndex(function (category) {
                  return category === group;
                });

                if (gMetric.key.startsWith("r")) {
                  // read metrics start with 'r + category's name, so put them in the Read series
                  chartSeries[0].data[groupIndex] = metricValue;
                } else if (gMetric.key.startsWith("w")) {
                  // write metrics starts with 'w + category's name, so put them in the Write series
                  chartSeries[1].data[groupIndex] = metricValue;
                }
              } else {
                // for histogram graph without categories we need only one series with data in type
                // [[metric, value],[metric, value]...]
                var value = [gMetric.label, metricValue];
            ...
            key: "createDefaultHistogramSeries",
            value: function createDefaultHistogramSeries(chartSeries) {
              if (chartSeries.length === 0) {
                chartSeries.push({
                  name: _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.monitor.performance.io.insight.histogram.series.read"),
                  data: []
                });
                chartSeries.push({
                  name: _util_vsan_util__WEBPACK_IMPORTED_MODULE_8__["VsanUiUtils"].getString("vsan.monitor.performance.io.insight.histogram.series.write"),
                  data: []
                });
              }
            }
            /**
             * For some graphs like dom-world-cpu or lsom-world-cpu, they have duplicated metric keys,
             * but the data are different even the keys are the same, we need a subKey to identify them
             */

          }, {
            key: "getSeriesLabel",
            value: function getSeriesLabel(key, subKey) {
              return key + (subKey ? " " + subKey : "");
            }
          }, {
            key: "isRealtimeSupportedView",
            value: function isRealtimeSupportedView(selectedView) {
              return PerfChartUtil.HIGH_RESOLUTION_SUPPORTED_VIEWS.some(function (view) {
                return view === selectedView;
              });
            }
          }, {
            key: "getAggregatedVsanPerfVmMode",
            value: function getAggregatedVsanPerfVmMode(moRef) {
              return _service_managed_object__WEBPACK_IMPORTED_MODULE_5__["ManagedObjectUtils"].isHost(moRef) ? _performance_view_perf_vm_mode_selector_perf_vm_mode__WEBPACK_IMPORTED_MODULE_11__["VsanPerfVmMode"].HOST_AGGREGATION : _performance_view_perf_vm_mode_selector_perf_vm_mode__WEBPACK_IMPORTED_MODULE_11__["VsanPerfVmMode"].CLUSTER_AGGREGATION;
            }
          }]);
        ...
        PerfChartUtil.RED_THRESHOLD_COLOR = "#F5DBD9";
        PerfChartUtil.YELLOW_THRESHOLD_COLOR = "#FCFFC5";
        PerfChartUtil.BLACK_METRIC_LEGEND = "black";
        PerfChartUtil.LEGEND_COLOR = "color";
        PerfChartUtil.LEGEND_ID = "id";
        PerfChartUtil.LEGEND_LINKED_TO = "linkedTo";
        PerfChartUtil.LEGEND_SHOW_IN_LEGEND = "showInLegend";
        PerfChartUtil.LEGEND_DASH_STYLE = "dashStyle";
        PerfChartUtil.PERCENTAGE_TYPE = "percentage";
        PerfChartUtil.RATE_BYTES_TYPE = "rate_bytes";
        PerfChartUtil.SIZE_BYTES_TYPE = "size_bytes";
        PerfChartUtil.TIME_MS_TYPE = "time_ms";
        PerfChartUtil.PERMILLE_TYPE = "permille";
        PerfChartUtil.MBYTES_TYPE = "mbytes";
        PerfChartUtil.TIME_S_TYPE = "time_s";
        PerfChartUtil.NUMBER_TYPE = "number";
        PerfChartUtil.LEGEND_BOTTOM = "bottom";
        PerfChartUtil.LEGEND_RIGHT = "right";
        PerfChartUtil.CENTER_ALIGN = "center";
        PerfChartUtil.TOP_VALIGN = "top";
        PerfChartUtil.HORIZONTAL_LAYOUT = "horizontal";
        PerfChartUtil.VERTICAL_LAYOUT = "vertical";
        PerfChartUtil.HISTOGRAM_LABEL_ROTATION = -50;
        PerfChartUtil.CHART_HEIGHT = 200; // "short": "18/4/2018 12:05 PM"

        PerfChartUtil.SHORT_PATTERN = "short"; // "show with seconds": "equivalent to 'M/d/yy, h:mm:ss a' 18/4/2018 12:05:33 PM.

        PerfChartUtil.SHORT_WITH_SECONDS_PATTERN = "M/d/yy, h:mm:ss a"; // "shortTime": "12:05 PM"

        PerfChartUtil.SHORT_TIME_PATTERN = "shortTime"; // "medium" : equivalent to 'h:mm:ss a' 9:03:01 AM.

        PerfChartUtil.MEDIUM_TIME_PATTERN = "mediumTime"; // "shortDate": "18/4/2018"

        PerfChartUtil.SHORT_DATE_PATTERN = "shortDate";
        PerfChartUtil.WARNING_AREA_NAME = "Polygons";
        PerfChartUtil.HIGH_RESOLUTION_SUPPORTED_VIEWS = [_performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].CLUSTER_VM_CONSUMPTION, _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].CLUSTER_BACKEND, _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].HOST_BACKEND, _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].HOST_VM_CONSUMPTION, _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].HOST_DISK, _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].HOST_PHYSICAL_ADAPTER, _performance_model_perf_monitor_view_type_model__WEBPACK_IMPORTED_MODULE_10__["PerfMonitorViewType"].HOST_VIRTUAL_ADAPTER];
        PerfChartUtil.LEGEND_ITEM_STYLE = {
          fontSize: "11px",
          fontWeight: "normal"
        };
        PerfChartUtil.BOTTOM_LEGEND_OPTION = {
          layout: PerfChartUtil.HORIZONTAL_LAYOUT,
          align: PerfChartUtil.CENTER_ALIGN,
          verticalAlign: PerfChartUtil.LEGEND_BOTTOM,
          borderWidth: 0,
          itemStyle: PerfChartUtil.LEGEND_ITEM_STYLE
        };
        PerfChartUtil.RIGHT_LEGEND_OPTION = {
          layout: PerfChartUtil.VERTICAL_LAYOUT,
          align: PerfChartUtil.LEGEND_RIGHT,
          verticalAlign: PerfChartUtil.TOP_VALIGN,
          borderWidth: 0,
          width: 160,
          itemStyle: PerfChartUtil.LEGEND_ITEM_STYLE
        };
        PerfChartUtil.NO_LEGEND_OPTION = {
          enabled: false
        };

        PerfChartUtil.toPercentages = function (permilles) {
          return permilles > 0 ? permilles / 10 : 0;
        };

        PerfChartUtil.toPermilles = function (percentages) {
          return percentages * 10;
        };

        PerfChartUtil.getGraphDefinition = function (graph) {
          var perfGraph = new _perf_graph__WEBPACK_IMPORTED_MODULE_12__["PerfGraph"]();
          perfGraph.label = graph.name;
          perfGraph.description = graph.description;
          perfGraph.unit = graph.unit;
          perfGraph.id = graph.id;

          if (graph.secondGraph != null) {
            perfGraph.secondGraph = PerfChartUtil.getGraphDefinition(graph.secondGraph);
          }

          if (graph.threshold) {
            perfGraph.threshold = _generated_perf_graph_threshold__WEBPACK_IMPORTED_MODULE_2__["PerfGraphThreshold"].Factory.create(graph.id, "", graph.threshold.yellow, graph.threshold.red, graph.threshold.direction);
          }

          perfGraph.metrics = graph.metrics.map(function (metrics) {
            return new _perf_graph__WEBPACK_IMPORTED_MODULE_12__["PerfGraphMetrics"](metrics.label, metrics.name, metrics.group, null, null, metrics.unit);
          });
          return perfGraph;
        };
      ...
      var PerfChartDataConstants = /*@__PURE__*/function () {
        var PerfChartDataConstants = /*#__PURE__*/_createClass(function PerfChartDataConstants() {
          _classCallCheck(this, PerfChartDataConstants);
        });

        PerfChartDataConstants.IOINSIGHT = "ioinsight";
        PerfChartDataConstants.CLUSTER_VSAN_CONFIGINFO_UUID_PROPERTY = _service_client_dataservice_data_service_property__WEBPACK_IMPORTED_MODULE_4__["ClusterComputeResource"].configurationEx.vsanConfigInfo.defaultConfig.uuid;
        PerfChartDataConstants.HOST_VSAN_CONFIGINFO_NODEUUID_PROPERTY = _service_client_dataservice_data_service_property__WEBPACK_IMPORTED_MODULE_4__["HostSystem"].config.vsanHostConfig.clusterInfo.nodeUuid;
        PerfChartDataConstants.VM_CONFIG_INSTANCE_UUID_PROPERTY = _service_client_dataservice_data_service_property__WEBPACK_IMPORTED_MODULE_4__["VirtualMachine"].config.instanceUuid;
        PerfChartDataConstants.CLUSTER_VM_CONSUMPTION_ENTITY = "com.vmware.vsan.perf.entity.cluster-domclient";
        PerfChartDataConstants.CLUSTER_VM_CONSUMPTION_REMOTE_ENTITY = "com.vmware.vsan.perf.entity.cluster-remotedomclient";
        PerfChartDataConstants.CLUSTER_COMPUTE_ONLY_VM_REMOTE_CONSUMPTION_ENTITY = "com.vmware.vsan.perf.entity.computeCluster-remotedomclient";
        PerfChartDataConstants.CLUSTER_BACKEND_ENTITY = "com.vmware.vsan.perf.entity.cluster-domcompmgr";
        PerfChartDataConstants.CLUSTER_FILE_SERVICE_ENTITY = "com.vmware.vsan.perf.entity.vsan-file-service";
        PerfChartDataConstants.HOST_VM_CONSUMPTION_ENTITY = "com.vmware.vsan.perf.entity.host-domclient";
        PerfChartDataConstants.HOST_BACKEND_ENTITY = "com.vmware.vsan.perf.entity.host-domcompmgr";
        PerfChartDataConstants.HOST_NETWORK_ENTITY = "com.vmware.vsan.perf.entity.vsan-host-net";
        PerfChartDataConstants.HOST_ISCSI_ENTITY = "com.vmware.vsan.perf.entity.vsan-iscsi-host";
        PerfChartDataConstants.HOST_VSAN_DIRECT_ENTITY = "com.vmware.vsan.perf.entity.vsan-direct-host";
        PerfChartDataConstants.VM_CONSUMPTION_ENTITY = "com.vmware.vsan.perf.entity.vm";
        PerfChartDataConstants.HOST_CAPACITY_DISK_ENTITY = "com.vmware.vsan.perf.entity.capacity-disk";
        PerfChartDataConstants.HOST_CACHE_DISK_ENTITY = "com.vmware.vsan.perf.entity.cache-disk";
        PerfChartDataConstants.HOST_DISK_GROUP_ENTITY = "com.vmware.vsan.perf.entity.disk-group";
        PerfChartDataConstants.HOST_PHYSICAL_ADAPTER_ENTITY = "com.vmware.vsan.perf.entity.vsan-pnic-net";
        PerfChartDataConstants.HOST_VIRTUAL_ADAPTER_ENTITY = "com.vmware.vsan.perf.entity.vsan-vnic-net";
        PerfChartDataConstants.HOST_RDT_NET_ENTITY = "com.vmware.vsan.perf.entity.rdt-net";
        PerfChartDataConstants.VM_VIRTUAL_DISK_ENTITY = "com.vmware.vsan.perf.entity.virtual-disk";
        PerfChartDataConstants.VM_VIRTUAL_SCSI_ENTITY = "com.vmware.vsan.perf.entity.vscsi";
        PerfChartDataConstants.CLUSTER_ISCSI_TARGET_ENTITY = "com.vmware.vsan.perf.entity.vsan-iscsi-target";
        PerfChartDataConstants.CLUSTER_ISCSI_LUN_ENTITY = "com.vmware.vsan.perf.entity.vsan-iscsi-lun";
        PerfChartDataConstants.IO_INSIGHT_ENTITY = "com.vmware.vsan.perf.entity.vmdk-ioinsight";
        PerfChartDataConstants.PMEM_CLUSTER_ENTITY = "com.vmware.vsan.perf.entity.vsan-pmem-cluster";
        PerfChartDataConstants.CLUSTER_VSAN_DIRECT_ENTITY = "com.vmware.vsan.perf.entity.vsan-direct-cluster";
        PerfChartDataConstants.PMEM_HOST_ENTITY = "com.vmware.vsan.perf.entity.vsan-pmem-host";
        PerfChartDataConstants.VSAN_MAX_DISK_SCSIFW = "com.vmware.vsan.perf.entity.vsan2-disk-scsifw";
        PerfChartDataConstants.VSAN_MAX_DISK_LSOM = "com.vmware.vsan.perf.entity.vsan2-disk-lsom2"; // historical space report view

        PerfChartDataConstants.HISTORICAL_SPACE_REPORT = "com.vmware.vsan.perf.entity.vsan-cluster-capacity";
        PerfChartDataConstants.HIGH_RESOLUTION_PREFIX = "hr-";
        return PerfChartDataConstants;
      }();
      /***/
    ...
    "FAE6":
    /*!*****************************************************************************!*\
      !*** ./src/app/vsan/performance/view/perf-vm-mode-selector/perf-vm-mode.ts ***!
      \*****************************************************************************/

    /*! exports provided: VsanPerfVmMode */

    /***/
    function FAE6(module, __webpack_exports__, __webpack_require__) {
      "use strict";
      ...
      __webpack_require__.d(__webpack_exports__, "VsanPerfVmMode", function () {
        return VsanPerfVmMode;
      });
      /* Copyright 2022 VMware, Inc. All rights reserved. -- VMware Confidential */

      /**
       * Different VM chart modes.
       */


      var VsanPerfVmMode = /*@__PURE__*/function (VsanPerfVmMode) {
        VsanPerfVmMode["CLUSTER_AGGREGATION"] = "CLUSTER_AGGREGATION";
        VsanPerfVmMode["HOST_AGGREGATION"] = "HOST_AGGREGATION";
        VsanPerfVmMode["PER_VM"] = "PER_VM";
        VsanPerfVmMode["TOP_CONTRIBUTORS"] = "TOP_CONTRIBUTORS";
        return VsanPerfVmMode;
      }({});
      /***/
    ...
    /***/
    function NvCx(module, __webpack_exports__, __webpack_require__) {
      "use strict";
      ...
      __webpack_require__.d(__webpack_exports__, "PerfMonitorViewType", function () {
      ...
      var PerfMonitorViewType = /*@__PURE__*/function (PerfMonitorViewType) {
        ...
        PerfMonitorViewType[PerfMonitorViewType["CLUSTER_VSAN_DIRECT"] = 8] = "CLUSTER_VSAN_DIRECT"; // Host
        ...
        PerfMonitorViewType[PerfMonitorViewType["HOST_PMEM"] = 17] = "HOST_PMEM"; // Virtual Machine
    ...
    "YGYr":
    /*!************************************************************!*\
      !*** ./src/app/vsan/performance/model/base-perf-entity.ts ***!
      \************************************************************/

    /*! exports provided: BasePerfEntity */

    /***/
    function YGYr(module, __webpack_exports__, __webpack_require__) {
      "use strict";
      ...
      __webpack_require__.d(__webpack_exports__, "BasePerfEntity", function () {
        return BasePerfEntity;
      });
      /**
       * This object is used in the most of the single entity perf pages, and some of the multi-entities
       * pages need to extend this object to display the metrics and the charts
       */


      var BasePerfEntity = /*#__PURE__*/function () {
        function BasePerfEntity() {
          _classCallCheck(this, BasePerfEntity);
        }

        _createClass(BasePerfEntity, [{
          key: "entityType",
          get:
          /**
           * This is the keyword for backend side to query the data. It's defined in the configSpec.
           */
          function get() {
            return this.configSpec ? this.configSpec.name : null;
          }
          /**
           * This method is used to get the key for ConsolidatedViewHelper.entityVisibilityStateMap
           * and PerfOrchestratorService.entitySelectionStateMap. We need this because now
           * PerfOrchestratorService is also consumed by File share performance. In this view the
           * specifiedObjectRef is not set.
           */

        }, {
          key: "entityIdentifier",
          get: function get() {
            if (this.specifiedObjectRef) {
              return this.specifiedObjectRef.value;
            }
          ...
          value: function equals(that) {
            ...
            return this.entityId === that.entityId && this.entityUuid === that.entityUuid && this.configSpec === that.configSpec;
          }
        }]);
    ...
    "lXqx":
    /*!***********************************************************************!*\
      !*** ./src/app/vsan/common/component/chart/performance/perf-graph.ts ***!
      \***********************************************************************/

    /*! exports provided: PerfGraph, PerfGraphMetrics */

    /***/
    function lXqx(module, __webpack_exports__, __webpack_require__) {
      "use strict";
      ...
      __webpack_require__.d(__webpack_exports__, "PerfGraph", function () {
        return PerfGraph;
      });
      /* harmony export (binding) */


      __webpack_require__.d(__webpack_exports__, "PerfGraphMetrics", function () {
        return PerfGraphMetrics;
      });
      /**
       * The definition of a perf chart
       */


      var PerfGraph = /*@__PURE__*/function () {
        var PerfGraph = /*#__PURE__*/function () {
          function PerfGraph() {
            _classCallCheck(this, PerfGraph);

            /**
             * In performance chart view, all the charts are pivotal by default.
             * But several charts in the Performance for support view are not pivotal,
             * they should be displayed only when user decides to see the detailed data.
             */
            this.isPivotalGraph = true;
          }

          _createClass(PerfGraph, [{
            key: "isHistogram",
            get: function get() {
              return this.id.endsWith("histogram");
            }
          }]);
        ...
        PerfGraph.createPerfGraph = function (id, label, unit, metrics, threshold) {
          var isPivotal = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : true;
          var graph = new PerfGraph();
          graph.id = id;
          graph.label = label;
          graph.metrics = metrics;
          graph.threshold = threshold;
          graph.unit = unit;
          graph.isPivotalGraph = isPivotal;
          return graph;
        };
      ...
      /**
       * Defines a single line in a perf chart, usually a perf chart contains several lines
       */


      var PerfGraphMetrics = /*#__PURE__*/_createClass(
      /**
       *
       * @param key: The key of the line, we will use this key to find the actual chart data
       * @param label: display name of the chart line, using for chart legend
       * @param group: The group label of the performance metric.
       * @param description: line description, currently not used.
       * @param aggregatedBy: The value of this metrics is aggregated by the specified metrics values
       * @param unit: Specified unit for the metric. Some graphs can have metrics with different unit types.
       */
      function PerfGraphMetrics(key, label, group, description, aggregatedBy, unit) {
        _classCallCheck(this, PerfGraphMetrics);
    ...
    "w4Wc":
    /*!***********************************************************************************!*\
      !*** ./src/app/vsan/common/component/chart/model/metric-comparisson-type.data.ts ***!
      \***********************************************************************************/

    /*! exports provided: MetricComparissonType */

    /***/
    function w4Wc(module, __webpack_exports__, __webpack_require__) {
      "use strict";
      ...
      __webpack_require__.d(__webpack_exports__, "MetricComparissonType", function () {
        return MetricComparissonType;
      });
      /* Copyright 2020 VMware, Inc. All rights reserved. -- VMware Confidential */


      var MetricComparissonType = /*@__PURE__*/function (MetricComparissonType) {
        MetricComparissonType[MetricComparissonType["EQUAL"] = 0] = "EQUAL";
        MetricComparissonType[MetricComparissonType["GREATER"] = 1] = "GREATER";
        MetricComparissonType[MetricComparissonType["EQUAL_OR_GREATER"] = 2] = "EQUAL_OR_GREATER";
        MetricComparissonType[MetricComparissonType["LESS"] = 3] = "LESS";
        MetricComparissonType[MetricComparissonType["EQUAL_OR_LESS"] = 4] = "EQUAL_OR_LESS";
        return MetricComparissonType;
      }({});
      /***/
